...
public class RolesFirstImpl3p2 implements QuasiRandomMatchGenerator {
    ...
    public List<List<List<TPlayer>>> generateMatchups(List<TPlayer> players,
        ...
        int rotationMultiplier = (playersPerMatch+1)/2;
        ...
        for (int roundNum = 0; roundNum < numMatches; roundNum++) {
            ...
            for (TPlayer player : players) {
                Preconditions.checkState(roleAssignmentsByRound.containsKey(player));
            ...
            for (int matchNum = 0; matchNum < matchesPerRound; matchNum++) {
                ...
                for (int role = 0; role < playersPerMatch; role++) {
                    List<TPlayer> playersForRole = playersByRole.get(role);
                    //Rotate to not keep leaving last players "stranded"
                    List<TPlayer> rotatedPlayersForRole = Lists.newArrayList(playersForRole);
                    Collections.rotate(rotatedPlayersForRole, roundNum*rotationMultiplier);
                    TPlayer player = choosePlayerWithFewestMatchesAgainst(match, rotatedPlayersForRole, matchupCounts);
                    match.add(player);
    ...
    private Map<TPlayer, List<Integer>> getRoleAssignmentsByRound(
        ...
        for (int i = 0; i < firstRoundWithBye.size(); i++) {
            ...
            for (int playerIdx = 0; playerIdx < match.size(); playerIdx++) {
                ...
                //i.e. have additional permutations used in those cases instead of repeating existing
                //permutations
